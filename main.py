...
def eliminarViaje():
    ...
    if len(viajes) == 0:
        ...
        for i, viaje in enumerate(viajes, start=1):
            print(i, "desde", viaje["origen"], "hasta", viaje["destino"], "fecha", viaje["fecha"])

        numeroAEliminar = int(input("\nQué número de viaje desea eliminar?: "))
        if 1 <= numeroAEliminar <= len(viajes):
            eliminado = viajes.pop(numeroAEliminar - 1)
            print("\nSe eliminó el viaje desde", eliminado["origen"], "hasta", eliminado["destino"], "el", eliminado["fecha"])
        else:
